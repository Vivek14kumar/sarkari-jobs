...
export default function JobClient({ job }) {
  ...
  return (
    <main className="max-w-6xl mx-auto bg-white shadow-lg rounded-2xl p-6 md:p-10 my-8 border border-gray-200">
      ...
      {description && (
        <section className="mb-6 bg-gray-50 p-5 rounded-lg border-l-4 border-blue-400 shadow-sm">
          <h3 className="text-xl font-semibold text-gray-800 mb-3 flex items-center gap-2">
            <FaInfoCircle className="text-blue-600" /> {language === "en" ? "Description" : "विवरण"}
          </h3>
      ...
      {job.eligibility && (
        <section className="mb-6 bg-gray-50 p-5 rounded-lg border-gray-500 border-l-4 shadow-sm">
          ...
          {(() => {
            ...
            <p key={`elig-${index}`} className="text-gray-800 mb-2 leading-relaxed">
              {part}
            </p>
          ))}
        </section>
      ...
      {ageLimits.length > 0 && (
        <section className="mb-6 bg-yellow-50 p-5 rounded-lg border-l-4 border-yellow-400 shadow-sm">
          <h3 className="text-xl font-semibold text-gray-800 mb-2 flex items-center gap-2">
            <FaUsers className="text-yellow-600" /> {language === "en" ? "Age Limit" : "आयु सीमा"}
          </h3>
          {ageLimits.map((age) => (
            <div key={age._id} className="mb-4">
              <p className="text-gray-700">
                {age.min && (
                  <span className="mr-4">
                    <strong className="text-gray-700">{language === "en" ? "Minimum Age:" : "न्यूनतम आयु:"}</strong>{" "}
                    {age.min}
                  </span>
                )}
                {age.max && (
                  <span>
                    <strong className="text-gray-700">{language === "en" ? "Maximum Age:" : "अधिकतम आयु:"}</strong>{" "}
                    {age.max}
                  </span>
                )}
              </p>
              {age.relaxation && (
                <div className="mt-3">
                  <h4 className="font-semibold text-gray-800 text-sm mb-1 flex items-center gap-1">
                    <FaUsers className="text-blue-500 text-sm" /> {language === "en" ? "Age Relaxation" : "आयु में छूट"}
                  </h4>
                  <p className="text-gray-700 text-sm leading-relaxed">{age.relaxation}</p>
                </div>
              )}
            </div>
          ))}
      ...
      {job.extra_info?.length > 0 && (
        <section className="mb-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {job.extra_info.map((info) => {
            ...
            return (
              <div key={info._id} className="flex flex-col gap-2 bg-indigo-50 p-4 rounded-lg shadow-sm border-l-4 border-indigo-400 hover:shadow-md transition">
      ...
      {(job.payment_modes?.length > 0 || job.application_fees?.length > 0 || job.feeLastDate) && (
        <section className="mb-6 bg-green-50 p-5 rounded-lg border-l-4 border-green-400 shadow-sm">
          <h3 className="text-xl font-semibold text-gray-800 mb-2 flex items-center gap-2">
            <FaMoneyBillWave className="text-green-600" /> {language === "en" ? "Application Fee & Payment Modes" : "आवेदन शुल्क और भुगतान के तरीके"}
          </h3>
          ...
          {job.application_fees?.length > 0 ? (
            <ul className="list-disc list-inside text-gray-700 space-y-1 mb-3">
              {job.application_fees.map((fee, i) => (
                <li key={`fee-${i}`}>
                  {language === "en" ? `${fee.category}: ₹${fee.fee}` : `${fee.category} : ₹${fee.fee}`}
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-gray-700 mb-3">{language === "en" ? "No fee information available." : "शुल्क की जानकारी उपलब्ध नहीं है।"}</p>
          )}

          {job.feeLastDate && (
            <p className="text-gray-700 mb-3">
              <strong>{language === "en" ? "Fee Payment Last Date:" : "शुल्क भुगतान की अंतिम तिथि:"}</strong> {feeLastDate}
            </p>
          )}

          {job.payment_modes?.length > 0 && (
            <div className="text-gray-700">
              <strong>{language === "en" ? "Payment Modes:" : "भुगतान के तरीके:"}</strong> {job.payment_modes.join(", ")}
            </div>
          )}
        </section>
      )}
      ...
      {job.vacancy_table?.length > 0 && (
        <section className="mb-6">
          <h3 className="text-xl font-semibold text-gray-800 mb-3 flex items-center gap-2">
            <FaBook className="text-blue-600" /> {language === "en" ? "Vacancy Details" : "रिक्ति विवरण"}
          </h3>
          <div className="overflow-x-auto border rounded-lg shadow-sm">
            <table className="min-w-full text-sm text-gray-700 border-collapse">
              <thead className="bg-blue-700 text-white">
                <tr>
                  <th className="px-4 py-2 text-left">{language === "en" ? "Post Name" : "पद का नाम"}</th>
                  <th className="px-4 py-2 text-left">{language === "en" ? "Category" : "श्रेणी"}</th>
                  <th className="px-4 py-2 text-left">{language === "en" ? "No. of Posts" : "पदों की संख्या"}</th>
                </tr>
              </thead>
              <tbody>
                {job.vacancy_table.map((v, i) => (
                  <tr key={v._id || `vac-${i}`} className={`${i % 2 === 0 ? "bg-white" : "bg-blue-50"} hover:bg-blue-100`}>
          ...
          <p className="text-gray-700 mt-2 font-medium">
            <strong>{language === "en" ? "Total Posts:" : "कुल पद:"}</strong> {job.totalPosts}
          </p>
        </section>
      )}
      ...
      <section className="flex flex-wrap gap-4 mt-6">
        {job.applyLink && (
          <a href={job.applyLink} target="_blank" rel="noopener noreferrer" className="bg-green-600 text-white px-6 py-3 rounded-lg text-sm hover:bg-green-700 transition flex items-center gap-2">
            <FaCheckCircle /> {language === "en" ? "Apply Online" : "ऑनलाइन आवेदन करें"}
          </a>
        )}

        {job.officialLink && (
          <a href={job.officialLink} target="_blank" rel="noopener noreferrer" className="bg-blue-700 text-white px-6 py-3 rounded-lg text-sm hover:bg-blue-800 transition flex items-center gap-2">
            <FaLink /> {language === "en" ? "Official Website" : "आधिकारिक वेबसाइट"}
          </a>
        )}
      </section>
    </main>
