...
export default function AdmitCardsClient({ initialCards = [] }) {
  const [cards, setCards] = useState(initialCards);
  const [visibleCount, setVisibleCount] = useState(6);

  const visibleItems = cards.slice(0, visibleCount);

  if (!cards.length) return <p className="text-center py-10 text-gray-500">No Admit Cards found.</p>;
    ...
    <div>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {visibleItems.map((item) => {
          ...
          const buttonHref = item.link?.trim() ? item.link : "/admit-cards";
            ...
            <div key={item._id} className="relative group bg-white rounded-2xl overflow-hidden shadow-lg border border-gray-100 hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
              ...
              <div className="p-5 relative z-10 top-1">
                ...
                <div className="flex flex-wrap gap-2">
                  <span className="text-xs font-medium bg-gradient-to-r from-blue-100 to-indigo-200 text-blue-800 px-2 py-1 rounded-full shadow-sm">{item.category || "Admit Card"}</span>
              ...
              <div className="border-t p-4 bg-gray-50 flex justify-center relative z-10">
                <Link href={buttonHref} className="w-full flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white py-2 rounded-xl font-semibold shadow-md transition duration-300">Download Admit Card</Link>
              </div>
            </div>
          );
        })}
      ...
      {visibleCount < cards.length && (
        <div className="flex justify-center mt-6">
          <button onClick={() => setVisibleCount(prev => prev + 6)} className="bg-green-600 hover:bg-green-700 text-white px-8 py-2 rounded-xl font-semibold shadow">
            Load More
          </button>
        </div>
      )}
